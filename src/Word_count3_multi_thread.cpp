...
int main(int argc, char **argv) {
    if (argc != 2) {
        try {
            ...
            std::cout << "Incorrect number of arguments:" << std::endl << "./Word_count3_multi_thread <config path>" <<std::endl;
            exit(1);
        }
    }
    ...
    int merger_threshold = config_object.get_int("merger_threshold");
    ...
    for (boost::filesystem::directory_entry& entry : boost::filesystem::recursive_directory_iterator(in_file)) {
        ...
        if (text_results.size() == merger_threshold) {
